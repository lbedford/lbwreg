<?php 
  ...
  $result = mysql_query("SELECT * FROM Events WHERE id=$userforum", $db);
  ...
  if (isset($submit)) {
    switch ($submit)
    {
      ...
      case "SAVE":
        ...
        $ref = header ("Location: messages.php?submit=browse");
        exit ();
      ...
      case "read":
        ...
        exit ();
      case "delete":
        $discussion_result = mysql_query ("SELECT * FROM discussions WHERE id=$number", $db);
        $msg = mysql_fetch_array ($discussion_result);
        $event_result = mysql_query ("SELECT * FROM Events WHERE id=".$msg["forum"], $db);
        $fi = mysql_fetch_array ($event_result);
        extract($fi, EXTR_SKIP);
        extract($msg, EXTR_SKIP);
        $messages--;
